...
interface EmptyProgressStatsProps {
  action?: ReactNode;
  icon?: ReactNode;
}

export function EmptyProgressStats({ action, icon }: EmptyProgressStatsProps) {
  const { theme } = useTheme();
      ...
      <CardContent>
        <DashboardEmpty
          icon={
            icon ?? (
              <Text
                style={{ color: theme.colors.onSurfaceVariant, fontSize: 22 }}
              >
                📊
              </Text>
            )
