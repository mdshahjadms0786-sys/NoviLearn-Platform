...
type FlowStatus = "setup" | "loading" | "error" | "ready" | "complete";
...
export function PracticeFlow({ initialTopic }: PracticeFlowProps) {
  const router = useRouter();
  const token = useAuthStore((s) => s.token);
  const { theme } = useTheme();

  const [status, setStatus] = React.useState<FlowStatus>("setup");
