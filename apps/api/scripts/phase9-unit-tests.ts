...
import { normalizeTopic } from "../src/utils/topic";
import {
  RESPONSE_DISCLAIMER,
  normalizeProviderResponse,
} from "../src/ai/normalizer";
import { normalizePracticeQuestions } from "../src/practice/practice.normalizer";
import { toClientPracticeSet } from "../src/practice/practice.types";
import { evaluateAnswer } from "../src/practice/evaluator";
import type {
  InternalPracticeQuestion,
  PracticeSessionRecord,
} from "../src/practice/practice.types";
...
const PRACTICE_CONFIG: PracticeConfig = {
...
check("keeps punctuation", normalizeTopic("Photosynthesis!"), "photosynthesis!");
...
expectThrows(
  ...
  () => normalizeProviderResponse("q", JSON.stringify({ foo: 1 })),
  502,
  "AI_RESPONSE_INVALID",
);
expectThrows(
...
expectThrows(
  ...
  () =>
    normalizeProviderResponse(
      "q",
      JSON.stringify({ explanation: "only explanation" }),
    ),
  502,
  "AI_RESPONSE_INVALID",
);
expectThrows(
  ...
  () => normalizeProviderResponse("q", JSON.stringify({ summary: "   " })),
  502,
  "AI_RESPONSE_INVALID",
);
...
const fullResponse = normalizeProviderResponse(
  ...
  JSON.stringify({
    ...
    visualRepresentation: {
      ...
      nodes: [{ label: "Sunlight" }, { label: "Chlorophyll" }],
      edges: [{ from: 0, to: 1, label: "absorbs" }],
    },
  }),
);
...
checkTrue("related concepts surfaced", fullResponse.relatedConcepts?.length === 1);
...
checkTrue(
  ...
  keyPointsSection !== undefined &&
    ...
    (keyPointsSection.items as string[]).length === 6,
);
checkTrue(
  "visual title kept",
...
check(
  "visual edge label kept",
  fullResponse.visualLearning?.edges,
  [{ from: 0, to: 1, label: "absorbs" }],
);
...
expectThrows(
  ...
  () =>
    normalizePracticeQuestions(
      PRACTICE_CONFIG,
      JSON.stringify({
        questions: [{ type: "mcq", question: "Q?", options: ["A", "B"] }],
      }),
    ),
  502,
);
...
expectThrows(
  ...
  () =>
    normalizePracticeQuestions(PRACTICE_CONFIG, JSON.stringify({
      questions: [
        {
          type: "mcq",
          question: "Same?",
...
check("true_false options enforced", dedupeQuestions[1]?.options, ["True", "False"]);
check(
...
const mcqQuestion: InternalPracticeQuestion = {
  id: "q1",
  type: "mcq",
  question: "Q?",
  options: ["Yes", "No"],
...
const tfQuestion: InternalPracticeQuestion = {
  id: "q2",
  type: "true_false",
  question: "Q?",
  options: ["True", "False"],
...
const saQuestion: InternalPracticeQuestion = {
  id: "q3",
  type: "short_answer",
  question: "Q?",
...
checkTrue("true_false canonical match passes", evaluateAnswer(tfQuestion, "true"));
checkTrue(
  "true_false wrong value fails",
  !evaluateAnswer(tfQuestion, "False"),
);
checkTrue(
...
const internalSession: PracticeSessionRecord = {
...
check(
  "visible question payload preserved",
  clientSet.questions,
  [{ id: "q1", type: "mcq", question: "Which pigment?", options: ["Chlorophyll", "Melanin"] }],
);
checkTrue("session id and topic preserved", clientSet.sessionId === internalSession.sessionId && clientSet.topic === "chlorophyll");
