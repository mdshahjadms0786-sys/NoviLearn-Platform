import { localEmbedInputs } from "../src/ai/embeddings/local.js";
import {
  buildOpenAiEmbeddingRequest,
  parseOpenAiEmbeddingResponse,
} from "../src/ai/embeddings/providers/openai.embeddings.js";
import { buildTutorMessages } from "../src/ai/prompts/system.js";
import {
  buildAnthropicRequest,
  parseAnthropicResponse,
} from "../src/ai/providers/anthropic.provider.js";
import { buildSuggestions } from "../src/progress/personalization.js";
import { toChunkInputs, splitIntoChunks } from "../src/rag/chunking.js";
import { toKnowledgeSources } from "../src/rag/context.js";
import { cosineSimilarity } from "../src/rag/vector.js";
import { slugifyTopic } from "../src/utils/topic.js";
let passed = 0;
let failed = 0;
...
check("user turn carries the student message single role", request.messages, [
  {
    role: "user",
    content: "Student question: What is photosynthesis?",
  },
]);
...
check(
  "model + input mapped",
  buildOpenAiEmbeddingRequest("text-embedding-3-small", ["a", "b"]),
  {
...
check(
  "wrong count -> null",
  parseOpenAiEmbeddingResponse(reorderedBody, 3),
  null,
);
...
check("identical inputs produce identical vectors", vectors[0], vectors[1]);
...
const longContent = Array.from(
  ...
  (_, i) => "Paragraph number " + (i + 1) + " about plant cells",
).join("\n\n");
...
checkTrue(
  ...
  chunks.every(
    (chunk) =>
      chunk.checksum.length === 32 && /^[0-9a-f]+$/.test(chunk.checksum),
  ),
);
...
check(
  ...
  splitIntoChunks("s", "One short paragraph.").length,
  1,
);
...
check("metadata forwarded onto every input", inputs[0]?.metadata, {
  grade: 7,
});
check(
  "source/title/topic forwarded",
  {
...
check(
  "slugifies spaces and punctuation",
  slugifyTopic("Cell Division!"),
  "cell-division",
);
check(
  "collapses internal whitespace",
  slugifyTopic("  Algebra   Basics  "),
  "algebra-basics",
);
check(
  "drops leading/trailing separators",
  slugifyTopic(" --extremes-- "),
  "extremes",
);
...
check(
  "excerpt matches short content",
  sources[0]?.excerpt,
  "A chloroplast is a double-membrane organelle.",
);
check(
  "fields forwarded",
  {
...
check(
  "continue uses most recent learning topic (Algebra)",
  suggestions[0]?.topic,
  "Algebra",
);
check(
  "practice uses most recent practice topic",
  suggestions[1]?.topic,
  "Cell biology",
);
check(
...
check(
  "related uses most recent related topic (Chloroplasts)",
  suggestions[3]?.topic,
  "Chloroplasts",
);
const masteredInput = {
...
check(
  "empty progress -> no suggestions",
  buildSuggestions(emptyInput).length,
  0,
);
...
const groundedMessage = buildTutorMessages("  What is photosynthesis?  ", {
  ...
  systemContext:
    'The following knowledge was retrieved... [1] "Light reactions"',
});
checkTrue(
...
const emptyContextMessage = buildTutorMessages("Question?", {
  sources: [],
  systemContext: "",
});
checkTrue(
