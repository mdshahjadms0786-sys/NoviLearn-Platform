...
function ok(description: string, condition: boolean, context?: any): void {
...
async function main(): Promise<void> {
  ...
  const emptySummary = await api(base, "/progress/summary", { token: tokenA });
  ok(
  ...
  ok(
    "empty topics list",
    (await api(base, "/progress/topics", { token: tokenA })).body.data
      .length === 0,
  );
  ok(
    "empty learning history",
    (await api(base, "/progress/history/learning", { token: tokenA })).body.data
      .length === 0,
  );
  ok(
    "empty practice history",
    (await api(base, "/progress/history/practice", { token: tokenA })).body.data
      .length === 0,
  );
  ok(
    "empty suggestions",
    (await api(base, "/progress/suggestions", { token: tokenA })).body.data
      .length === 0,
  );
  ...
  await prisma.practiceSession.create({
    data: {
      ...
      results: [
        { ...resultsShape[0], questionId: "q2", correct: false, answer: "X" },
      ],
    },
  });
  await prisma.practiceSession.create({
  ...
  ] as unknown as (typeof InternalPracticeQuestion)[];
  ...
  const summary = await api(base, "/progress/summary", { token: tokenA });
  ...
  ok(
    ...
    Array.isArray(s.recentActivity) &&
      ...
      s.recentActivity.some((r: any) => r.type === "learn") &&
      s.recentActivity.some((r: any) => r.type === "practice"),
  );

  console.log("Topic progress");
  const topics = await api(base, "/progress/topics", { token: tokenA });
  const topicsData = topics.body.data as any[];
  ...
  ok(
    ...
    learning.body.data.every(
      (a: any) => a.topic === "photosynthesis" || a.topic === "magnetism",
    ),
  );
  ...
  const kinds = suggestions.body.data.map((s: any) => s.kind) as string[];
  ok(
  ...
  ok(
    ...
    suggestions.body.data.some(
      (s: any) => s.kind === "review_weak_topic" && s.topic === "chemistry",
    ),
  );
  ok(
    "related_next_topic surfaces metadata related concept first",
    suggestions.body.data.some(
      (s: any) => s.kind === "related_next_topic" && s.topic === "Chloroplasts",
    ),
  );
  ...
  ok(
    "user B topics empty",
    (await api(base, "/progress/topics", { token: tokenB })).body.data
      .length === 0,
  );
  ...
  ok(
    ...
    (await api(base, "/progress/suggestions", { token: tokenB })).body.data
      .length === 0,
  );

  console.log("Validation");
  const badLimits = await Promise.all([
    api(base, "/progress/history/learning?limit=0", { token: tokenA }),
    api(base, "/progress/history/learning?limit=101", { token: tokenA }),
    api(base, "/progress/history/learning?limit=abc", { token: tokenA }),
    api(base, "/progress/history/practice?limit=-3", { token: tokenA }),
  ]);
  ok(
    "invalid limits -> 400",
  ...
  ok(
    ...
    (await api(base, "/practice/generate", { method: "POST", body: {} }))
      .status === 401,
  );
  ...
  ok(
    ...
    (await prisma.practiceSession.count({ where: { userId: userAId } })) === 4,
  );
