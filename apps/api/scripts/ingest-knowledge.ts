 ...
 *
 * Usage:
 *   tsx scripts/ingest-knowledge.ts --file ./knowledge.json
 *   tsx scripts/ingest-knowledge.ts --sample
 *
 * JSON file shape:
...
import { toChunkInputs } from "../src/rag/chunking";
import { ingestKnowledge } from "../src/rag/ingestion";
...
const SAMPLE_SOURCES: KnowledgeSourceEntry[] = [
  {
...
function parseArgs(args: string[]): { file?: string; sample?: boolean } {
  const result: { file?: string; sample?: boolean } = {};
...
function loadSources(args: { file?: string; sample?: boolean }): KnowledgeSourceEntry[] {
  if (args.sample === true) {
    return SAMPLE_SOURCES;
  }
  if (args.file === undefined) {
    throw new Error(
      "Provide --file <path> or --sample. Example: tsx scripts/ingest-knowledge.ts --sample",
    );
  }
  const path = resolve(process.cwd(), args.file);
  const raw = readFileSync(path, "utf8");
  return JSON.parse(raw) as KnowledgeSourceEntry[];
}
