...
function questionTokenQuery() {
  return "Where does a xylophone zygote go near a quasar";
}
async function main() {
  await import("../src/index.js");
  const { prisma } = await import("../src/prisma.js");
  const { createRecentDuplicateGuard } = await import("../src/utils/dedupe.js");
  const { buildGrounding } = await import("../src/rag/rag.service.js");
  const { buildPersonalContext } =
    await import("../src/rag/personal-context.js");
  const { toChunkInputs } = await import("../src/rag/chunking.js");
  const { ingestKnowledge } = await import("../src/rag/ingestion.js");
    ...
    Array.from(
      ...
      () =>
        api(base, "/ai/learn", {
          ...
          token: tokenA,
          body: {
            question: `Rate probe ${Math.random()}`,
          },
        }),
    ),
  );
  ...
  const otherUserProbe = await api(base, "/ai/learn", {
    ...
    token: tokenB,
    body: {
      question: "Is the limit per user?",
    },
  });
  check(
  ...
  try {
    ...
    const afterWindow = await api(base, "/ai/learn", {
      ...
      token: tokenA,
      body: {
        question: "Window should have reset",
      },
    });
    check(
  ...
  const guard = createRecentDuplicateGuard({
    windowMs: 10_000,
  });
  let nextCalls = 0;
  const fakeReqA = {
    ...
    body: {
      question: "Repeat me exactly",
    },
    ip: "1.2.3.4",
  };
  const res1 = new FakeResponse();
  await guard(fakeReqA, res1, () => {
    nextCalls += 1;
  });
    ...
    grounded.sources.length >= 1 &&
      grounded.sources.some(
        (source) =>
          ...
          source.title === "Phase10 Test Source",
      ),
    grounded.sources,
  );
  ...
  const spawned = spawnSync(
    process.execPath,
    ["scripts/phase10-restart-check.js"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
  ...
  console.log(
    "Personal context + topic catalog (owner scoped, recency ranked)",
  );
...
function checkTrueForGrounding(description, value) {
  if (value) {
    passed += 1;
    console.log(`  ok ${description}`);
  } else {
