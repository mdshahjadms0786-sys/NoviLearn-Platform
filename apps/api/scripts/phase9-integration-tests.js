...
async function main() {
  ...
  const { answerPracticeQuestion, completePracticeSession } =
  ...
  const tokenAFresh = relogin.body.data.token;
  check(
  ...
  check(
    ...
    (await prisma.learningActivity.count({
      where: {
        userId: userAId,
      },
    })) === 0,
  ...
  const topicsList = await api(base, "/progress/topics", {
    token: tokenAFresh,
  });
  check(
  ...
  check(
    ...
    (
      await api(base, "/progress/history/learning?limit=0", {
        token: tokenAFresh,
      })
    ).status === 400,
  );
  check(
    ...
    (
      await api(base, "/progress/history/practice/nope", {
        token: tokenAFresh,
      })
    ).status === 400,
  );
