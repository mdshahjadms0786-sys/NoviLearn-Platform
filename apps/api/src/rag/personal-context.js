 ...
 */
export async function buildPersonalContext(userId) {
  ...
  const weakTopics = (weak ?? [])
    .filter(
      (entry) =>
        entry._min?.accuracy !== null && (entry._min?.accuracy ?? 100) < 70,
    )
  ...
  const knownTopics = [
    ...new Set(
      [
        ...learned.map((item) => normalizeTopic(item.topic)),
        ...practiced.map((item) => normalizeTopic(item.topic)),
  ...
  return {
    knownTopics,
    practicedTopics: [
      ...new Set(practiced.map((item) => normalizeTopic(item.topic))),
    ].slice(0, MAX_KNOWN_TOPICS),
    weakTopics,
  };
}
