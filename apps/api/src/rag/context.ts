...
import type { KnowledgeChunkRecord, PersonalContext } from "./types";
...
function formatPersonalContext(personal: PersonalContext): string {
  const lines: string[] = [];
  if (personal.knownTopics.length > 0) {
    lines.push(`Topics this learner has studied before: ${personal.knownTopics.join(", ")}.`);
  }
  if (personal.practicedTopics.length > 0) {
    lines.push(`Topics this learner has practiced: ${personal.practicedTopics.join(", ")}.`);
  }
...
export function buildGroundingPrompt(question: string, systemContext: string): string {
